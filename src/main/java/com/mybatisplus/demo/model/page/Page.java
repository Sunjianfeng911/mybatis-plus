package com.mybatisplus.demo.model.page;
...
public class Page<T> implements IPage<T> {
  ...
  public Page(HttpServletRequest request) {
    this.current = ServletRequestUtils.getIntParameter(request, "current", (int) current);
    this.size = ServletRequestUtils.getIntParameter(request, "size", (int) size);
    /*this.orderBy =
    ServletRequestUtils.getStringParameter(request, "sortField", null) == null
        ? null
   ...
   * 设置需要进行正序排序的字段
   *
   * <p>Replaced:{@link #addOrder(OrderItem...)}
   *
   * @param ascs 字段
   * @return 返回自身
   * @deprecated 3.2.0
