package com.mybatisplus.demo.util.oss;
...
import com.mybatisplus.demo.util.oConvertUtils;
import java.io.IOException;
import java.util.UUID;
import org.apache.tomcat.util.http.fileupload.FileItemStream;
import org.springframework.web.multipart.MultipartFile;

/** @Description: 阿里云 oss 上传工具类(高依赖版) @Date: 2019/5/10 */
public class OssBootUtil {
  ...
  public static String upload(FileItemStream file, String fileDir) {
    ...
    try {
      ...
      PutObjectResult result =
          ossClient.putObject(bucketName, fileUrl.toString(), file.openStream());
      // 设置权限(公开读)
      ossClient.setBucketAcl(bucketName, CannedAccessControlList.PublicRead);
      if (result != null) {
        System.out.println("------OSS文件上传成功------" + fileUrl);
